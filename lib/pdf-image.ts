 ...
 * Uses PyMuPDF via a Python subprocess. Requires `pip install pymupdf`.
 */
export function renderPageToPngBase64(
  ...
  return new Promise((resolve, reject) => {
    const proc = spawn("python3", [SCRIPT, target, String(pageNumber), String(dpi)], {
    ...
    proc.on("error", (e) => reject(new Error(`Failed to launch python3: ${e.message}`)));
    proc.on("close", (code) => {
      if (code !== 0) {
        const hint = stderr.includes("No module named fitz")
          ? "OCR mode requires PyMuPDF. Install it with: pip install pymupdf"
          : stderr.trim();
